...
멀티레포의 이점(독립 릴리스·접근 제어·blast radius 분리)은 팀·다환경에서 나온다. 운영자 1명·클러스터 1개에선 그 이점이 발생하지 않고 레포 동기화·교차 PR·컨텍스트 전환 비용만 남는다. 반대로 한 레포에 프로비저닝→설정→배포가 모여 있으면 리뷰어가 플랫폼 수명주기 전체를 한 번에 읽는다 — "포폴 가치는 노드 수가 아니라 플랫폼 운영"이라는 ADR-0002의 논리와 일치한다.
