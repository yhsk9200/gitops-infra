...
- **포폴 목적 재평가** — 시뮬레이터는 "모델을 소비하는 제품" 하나를 보여주지만, MLOps는 학습→추적→등록→승격→서빙→모니터링→재학습 루프가 이 레포가 이미 구축한 자산(GitOps 파이프라인, Keycloak IAM, Prometheus/Grafana 관측, Telegram 알림, 백업 규율)을 **전부 소비**한다. 도구 나열이 아니라 "플랫폼 위에 플랫폼을 얹는 판단"을 보여주는 쪽이 테크리드 포폴 서사에 부합한다.
