...
**ADR-0008로 대체됨** (2026-07-10) — 시뮬레이터 제품 대신 MLOps 라이프사이클로 피봇. MLflow 중심·Kubeflow 보류·alias/tag 운영 모델은 ADR-0008이 승계, 시뮬레이터 UI/Backend와 "AI 전체 별도 레포" 구조는 폐기.
...
향후 이 플랫폼 위에 모델 시뮬레이터 서비스를 별도 레포로 개발할 계획입니다. 이 서비스는 AI 모델 파일을 사용하고, 특정 모델 버전을 기준으로 시뮬레이션을 수행하며, 결과를 검증/기록해야 합니다.
...
Simulator UI는 최종 사용자가 사용하는 제품 화면입니다. MLflow UI를 대체하는 것이 아니라, 시뮬레이션 업무 흐름을 제공해야 합니다.
...
- 여러 워크로드가 AI 플랫폼을 공용으로 사용하고 namespace/RBAC/quota 분리가 필요함
