...
| 항목 | 상태 |
|---|---|
| 클러스터 | **가동 중** — 노드 `144.24.81.104`(reserved public IP, 2026-07-03 전환. 구 IP `158.179.169.201`은 release됨) (4 OCPU/24GB arm64, k3s v1.32.13), ArgoCD v3.4.4, **앱 15/15 Synced/Healthy**(2026-07-19 실측 — platform-mlops-mlflow 추가), 상시 메모리 ~4.4Gi/24Gi(18%) |
...
| Alerting | v1 PrometheusRule 가동 중 (`platform-monitoring-rules`, wave 5) + **Alertmanager receiver 연결 완료** (2026-07-09, PR #23): 채널 Telegram, 봇 토큰은 SealedSecret(`alertmanager-telegram-token`)으로 격리. synthetic alert로 firing/resolved 메시지 모두 실 수신 확인. **첫 실데이터 노이즈 조정** (2026-07-11, PR #28): 메타 알림 null 매처를 `Watchdog\|InfoInhibitor` 정규식으로 확장 — pulse v2 롤아웃 CPU 스파이크로 CPUThrottlingHigh(info)가 InfoInhibitor를 세워 텔레그램 실 노이즈 발생, 차트 기본 라우트의 null 처리가 우리 route 통째 대체에서 빠져 있었음. 런타임 config 반영 실측 완료 |
